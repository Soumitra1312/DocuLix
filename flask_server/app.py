    ...
    try:
        ...
        if not is_legal or confidence < 0.6:
            return json.dumps({
                ...
                "error": "⚠️ Warning: This document does not appear to be a legal document.",
    ...
    try:
        ...
        for file in files:
            ...
            if not is_legal and confidence >= 0.7:
                print(f"⚠️ Warning: {file.filename} does not appear to be a legal document")
                return json.dumps({
                    "error": f"⚠️ Warning: The file '{file.filename}' does not appear to be a legal document.",
                    "success": False,
                    "is_legal_document": False,
                    "confidence_score": round(confidence * 100, 1),
                    "document_type": doc_type,
            ...
            if not is_legal and confidence >= 0.5:  # Medium confidence warning but still proceed
                print(f"⚠️ Possible non-legal document: {file.filename} - Type: {doc_type}")
            elif is_legal:
